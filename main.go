...
func attack(charcharName, charcharClass string) string {
	if charcharClass == "warrior" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charcharName, 5+randint(3, 5))
	}

	if charcharClass == "mage" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charcharName, 5+randint(5, 10))
	}

	if charcharClass == "healer" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charcharName, 5+randint(-3, -1))
	}
...
// обратите внимание на "if else" и на "else"
func special(charcharName, charcharClass string) string {
	if charcharClass == "warrior" {
		return fmt.Sprintf("%s применил специальное умение `Выносливость %d`", charcharName, 80+25)
	}

	if charcharClass == "mage" {
		return fmt.Sprintf("%s применил специальное умение `Атака %d`", charcharName, 5+40)
	}

	if charcharClass == "healer" {
		return fmt.Sprintf("%s применил специальное умение `Защита %d`", charcharName, 10+30)
	}
